...
function deleteUser(name, e) {
  ...
  if (confirm('Remove ' + name + '?')) {
    delete users[name];
    if (currentUser === name) currentUser = Object.keys(users)[0] || null;
    saveData();
    renderUsers();
    loadUserTasks();
  }
    ...
    return (
      '<div class="user-card ' + (active ? 'active' : '') + '">' +
        ...
        '<div class="user-actions">' +
          '<button class="btn-select" onclick=\'selectUser(' + JSON.stringify(name) + ')\'>' +
            (active ? 'Selected' : 'Select') +
          '</button>' +
          '<button class="btn-delete" onclick=\'deleteUser(' + JSON.stringify(name) + ', event)\'>Remove</button>' +
        '</div>' +
      '</div>'
    );
  }).join('');
...
function loadUserTasks() {
  document.querySelectorAll('input[data-task]').forEach(cb => {
    cb.checked = currentUser && users[currentUser] ? (users[currentUser].tasks[cb.dataset.task] || false) : false;
  });
  updateCounters();
}
...
document.addEventListener('DOMContentLoaded', () => {
  loadData();
  document.querySelectorAll('input[data-task]').forEach(cb => cb.addEventListener('change', handleTask));
  document.getElementById('userName').addEventListener('keypress', e => { if (e.key === 'Enter') addUser(); });
});

// Make sure inline onclick handlers can see them:
